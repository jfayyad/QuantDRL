...
def quantizeConvBnRelu(x, xq, block, scale_x, zp_x, num_bits, bn_fake=False):
    ...
    for i in range(W.shape[0]):
        w = quantize_tensor(layer.weight.data[i], num_bits=num_bits[0][0])  # Min and Max not required, not data dependent
...
def quantizeConvBn(x, xq, block, scale_x, zp_x, num_bits, bn_fake=False):
    ...
    zp_list = []
    for i in range(W.shape[0]):
        w = quantize_tensor(layer.weight.data[i],
                            num_bits=num_bits[0][0])  # Min and Max not required, not data dependent
        tensor_list.append(w.tensor.float())
        scale_list.append(w.scale)
        zp_list.append(w.zero_point)
