...
for i, (data, target, idx) in enumerate(test_loader):
    ...
    for filter_it in range(Conv1out.shape[1]):
        for index in range(idx.shape[0]):
            filter_sums_original[filter_it][index+(i*(idx.shape[0]))] = np.mean(Conv1out[index].detach().numpy()[filter_it], axis=0)
    ...
    for filter_iter in range(QConv1out.shape[1]):
        for indexq in range(idx.shape[0]):
            filter_sums_quantized[filter_iter][indexq+(i*(idx.shape[0]))] = np.mean(QConv1out[indexq].detach().numpy()[filter_iter], axis=0)
