...
if w_and_b:
    wandb.init(project = "ResNet18_SAC_imgNet",
...
num_itr = 1            # number of iterations on each episode
...
def getReward(bitwidth_, eps, itr):
    ...
    ####################### Accuracy reward #######################
    error = acc[0] - acc[1]
    if w_and_b: wandb.log({"acc_quant": acc[1], "eps_itr": eps+(itr-1)})
    ...
    if w_and_b: wandb.log({"avg_par": avg_par, "eps_itr": eps+(itr-1)})
    ...
    if w_and_b: wandb.log({"avg_act": avg_act, "eps_itr": eps+(itr-1)})
    ...
    if args.train:
        ...
        for ep in range(1, TRAIN_EPISODES+1):
            ...
            for i in range(1, num_itr+1):
                ...
                action_bits = np.floor((1.5 * np.array(action) + 6.5))

                # if ep < 5:
                #     action_bits = np.ones_like(action_bits) * 4#(9 - ep)
                #     action = [action_bits]
                ...
                r_acc, r_w, r_a, r_kl, r_lb = getReward(bitwidth, ep, i)
            ...
            if w_and_b: wandb.log({"episodic_reward": episodic_reward, "episode": ep})
            print('########################################################################')
